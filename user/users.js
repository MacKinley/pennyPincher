...
var User = function( userConfig ) {
  var mongoose = userConfig.modules.mongoose,
    userSchema = userConfig.modules.schema, 
    userModel = userSchema.userModel,
    _ = userConfig.modules.lodash, 
    Promise = userConfig.modules.promise, 
    supportedServices = 'google facebook'.split(' '), 
    database = userConfig.mongoRoot,
    error = userConfig.errors,
    products = userConfig.modules.products;

  mongoose.createConnection(database);

  function getUserIdString( service ) {
    var queryString;
  ...
  function parseGoogleLogin( profile, token ) {
    return new Promise ( function ( resolve, reject ) {

    });
  };
  function getUserFromGoogle( userId ) {
    return new Promise( function ( resolve, reject ) {
      userModel.find({'google.id': userId}, function ( err, user ) {
        if (err) {
          reject({
            err: errors.notFound,
            location: 'getUserFromGoogle'
          });
        } else {
          resolve(user);
        }
  ...
  return {
    ...
    getUserFromId: function( userId, service ) {
      return new Promise( function ( resolve, reject ) {
        switch(service) {
          case 'google':
    ...
    addUserSubscription: function( userId, asin, service ) {
      return new Promise( function ( resolve, reject ) {
    ...
    removeUserSubscription: function( userId, asin) {
      return new Promise( function ( resolve, reject ) {

      });
    },
    getAllSubscribedUsers: function( asin ) {
